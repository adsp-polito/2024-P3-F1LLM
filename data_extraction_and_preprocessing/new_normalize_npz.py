...
def normalize_data(df, scaler_type="MinMaxScaler", filtered = True, driver_failures = [], failures = [], year=0, event_name=""):
    ...
    df["Failures"] = "No Failures"
    index = 0
    if driver_failures:
        for driver in driver_failures:
            df.loc[df["DriverNumber"] == int(driver), "Failures"] = failures[index]
    ...
    index = 0
    for driver in driver_failures:
        normalized_data_driver = processed_data[processed_data[:, -9] == np.float32(driver)]
        print(f'Normalized data shape for {np.float32(driver)}: {normalized_data_driver.shape}')
...
def preprocessing_and_normalization(input_folder_path, output_folder_path = "normalized", scaler_type="MinMaxScaler", save_to_file=True):
    ...
    for file in os.listdir(input_folder_path):
        ...
        failures_df = pd.read_csv('../Failures/new_19-24_all_events_anomalies.csv')
        ...
        normalized_data = normalize_data(df, scaler_type, False, driver_failures,failures, year, event_name)
        np.savez_compressed(
