...
def compute_reconstruction_error(inputs, outputs):
    return torch.mean(torch.abs(inputs - outputs), dim=(1, 2)).detach().cpu().numpy()
...
def train_autoencoder(autoencoder, train_loader, val_loader, epochs, validation_freq, device, learning_rate, criterion,
    ...
    for epoch in range(1, epochs + 1):
        ...
        torch.save(autoencoder.state_dict(),
                   f"models/autoencoder_{optimizer_name}_lr{learning_rate_str}_ep{epoch}_loss{train_loss:.4f}.pth")
        print("Model saved.")
    return history
